...
def runServer():
    ...
    while 1:
        ...
        if data=="quit":
            ...
            msg = "Port is open. Press ENTER to send another message or type 'quit' to end connection."
            msg2 = "Port is closed. Press ENTER to send another message or type 'quit' to end connection."

            check = socket.socket.connect_ex(socket.gethostbyname(name), port)

            if check == socket.gethostbyname(name):
                client.send(msg.encode('ascii', 'strict'))
